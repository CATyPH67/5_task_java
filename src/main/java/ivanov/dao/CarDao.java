...
public class CarDao implements DAO<Long, Car> {
    ...
    public void update(Long id, Long accident) {
        try {
            PreparedStatement statement = connection.prepareStatement(
                    "UPDATE \"car\" SET accident_id = ? WHERE id = ?");

            statement.setInt(1, accident.intValue());
    ...
    public List<Optional<Car>> getByAccident(Long key) {
        List<Optional<Car>> cars = new ArrayList<>();
        try {
            PreparedStatement statement = connection.prepareStatement("SELECT * FROM \"car\" WHERE accident_id = ?");
            statement.setInt(1, key.intValue());
            ResultSet resultSet = statement.executeQuery();
